...
Try the stronger anchored-triangle route first:
...
M496 has `U*h=W*h` for every shared-step pair.  Proving this identity would
...
has short closure `ok`; both 60-second and 300-second size-9 rawmodel runs
timed out without finding a model.  This is not proof.
...
Its external cycle-end template style suggests naming the end and next-to-last positions of
