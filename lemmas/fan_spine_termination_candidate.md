...
Under good `P`, this sharpens to `w_q=P <=> T_q=h`: both possible fixers of
`P` contain the interval `P -> P -> P`, so two-step source reconstruction
identifies them. This is special fixed-point uniqueness, not general right
cancellativity.
